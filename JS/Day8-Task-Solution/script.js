...
import { greetUser } from "./utilis.js";
...
getFirstThreePostTitles().then(titles=>console.log(titles));
...
const result1 = safeParseJSON(validJSON);
console.log(result1); 

const result2 = safeParseJSON(invalidJSON);
console.log(result2);
...
import DataFetcher from './utilis.js';

const fetcher = new DataFetcher();

fetcher.getUser(1).then(user => {
  if (user) {
    console.log(`Name: ${user.name}`);
    console.log(`Email: ${user.email}`);
  }
